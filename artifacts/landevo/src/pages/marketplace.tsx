...
import { Map, List, MapPin, ShieldCheck, Check, Clock, ChevronDown, Loader2 } from "lucide-react";
...
function listingStatusBadge(status: ListingStatus) {
  ...
  return (
    <Badge variant="secondary" className="bg-slate-500 text-white border-none shadow-sm backdrop-blur-md bg-opacity-90 font-bold tracking-wide text-[10px] px-2 py-1 uppercase">
      {status}
    </Badge>
  );
}
...
export default function Marketplace() {
  ...
  const recentVerified = listings.filter(
    (l) => l.status === "verified" || l.status === "active"
  ).slice(0, 3);
      ...
      <div className="flex flex-col md:flex-row h-full w-full">
        ...
        <div className="w-full md:w-[280px] flex-shrink-0 bg-card border-r border-border p-6 overflow-y-auto">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-xs text-primary font-medium hover:underline" onClick={() => setSearch("")}>Clear All</button>
          </div>
          ...
          <div className="p-6 md:p-8 max-w-6xl mx-auto w-full">
            ...
            <div className="bg-card border rounded-lg p-3 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 shadow-sm">
              ...
              <div className="flex items-center gap-3 flex-1 max-w-xs ml-auto">
                ...
                />
                <Button size="sm" className="h-9 px-4 font-semibold flex-shrink-0">Search</Button>
              </div>
            </div>
                  ...
                  <Card key={listing.id} className="overflow-hidden hover:shadow-lg transition-all group flex flex-col h-full border-border/60">
                    <div className="aspect-[4/3] bg-muted relative overflow-hidden">
                      {listing.images && listing.images.length > 0 ? (
                        ...
                        <div className={`absolute inset-0 bg-gradient-to-br ${i % 3 === 0 ? 'from-green-900/40 to-slate-800/80' : i % 3 === 1 ? 'from-slate-700/60 to-teal-900/80' : 'from-emerald-900/40 to-slate-700/80'} group-hover:scale-105 transition-transform duration-500`}></div>
                      )}
                    ...
                    <CardContent className="p-5 flex flex-col flex-1">
                      <div className="mb-3">
                        <p className="text-xs font-bold tracking-wider text-muted-foreground uppercase mb-1">{listing.propertyType}</p>
                        <h3 className="font-bold text-lg leading-tight line-clamp-1 group-hover:text-primary transition-colors">{listing.title}</h3>
                      </div>
                      ...
                      <div className="mt-auto border-t border-border/50 pt-4 flex items-end justify-between">
                        <div>
                          <p className="text-xs text-muted-foreground font-medium mb-0.5">
                            {listing.areaSqm ? `${listing.areaSqm.toLocaleString()} sqm` : listing.propertyType}
                          </p>
                          <p className="font-bold text-xl text-foreground">{formatCurrency(listing.price)}</p>
                        </div>
                    ...
                    <Link key={`recent-${listing.id}`} href={`/marketplace/${listing.id}`}>
                      <div className="flex gap-4 bg-card p-3 rounded-lg border hover:border-primary/50 transition-colors cursor-pointer">
                        ...
                        <div className="flex flex-col justify-center">
                          ...
                          <p className="text-xs text-muted-foreground mb-1">{listing.location || listing.city}</p>
                          <p className="font-semibold text-sm text-primary">{formatCurrency(listing.price)}</p>
                        </div>
                      </div>
                    </Link>
                  ))}
