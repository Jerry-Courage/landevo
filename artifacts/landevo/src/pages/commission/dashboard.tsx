...
import { Clock, CheckCircle2, XCircle, ArrowUpRight, FileText } from "lucide-react";
...
export default function CommissionDashboard() {
  ...
  // monthlyActivity comes from the extended API response
  const chartData     = (dashboard as any)?.monthlyActivity ?? [];

  const pendingList  = recentVerifs.filter((v: any) => v.status === "pending" || v.status === "in_review");
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex gap-3">
            <Link href="/commission/verifications">
              <Button className="font-semibold h-9 bg-emerald-800 hover:bg-emerald-900">Review Verifications</Button>
            </Link>
          </div>
        </div>
              ...
              <div className="h-[200px]">
                {chartData.length === 0 ? (
                  <div className="h-full flex items-center justify-center text-sm text-muted-foreground">
                    Activity chart will populate as verifications are reviewed
                  </div>
                ) : (
          ...
          <CardContent className="p-0">
            {pendingList.length === 0 ? (
              ...
              <Table>
                ...
                <TableBody>
                  {pendingList.slice(0, 6).map((v: any) => (
                    <TableRow key={v.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <TableCell className="text-right pr-5">
                        <Link href="/commission/verifications">
                          <Button size="sm" className="h-7 text-xs font-semibold bg-emerald-700 hover:bg-emerald-800">Review</Button>
                        </Link>
                      </TableCell>
                    </TableRow>
                  ))}
